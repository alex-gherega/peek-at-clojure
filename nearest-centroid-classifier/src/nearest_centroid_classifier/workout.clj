...
;;
;; ({:id 0, :data ["4.8" "3.1" "1.6" "0.2"] :class "setosa"} ....)
...
(defn compute-centroid [data class-name]
  ...
  (let [ineteres-data  ...your-code;; first get the data filtered by class-name using filetr-class function we defined above
        interes-data (map :data filtered-class) ;; we remove the class name and id from our data
        cardinality ...your-code ;; count how many indexes we've got
        ]
    ;; compute miu vector for this class-name
    ;; * sum all vectors left in interes-data using sum-vectors
    ;; * divide all elements inside the resulting vector by cardinality
    ;; * make-it a vector (using vec)
    ;; return a map with keys :class and :miu 
    ....fn body))
